...
export function CustomClassPopover() {
    ...
    const handleQuickStyle = (className) => {
    ...
    const ClassPickerContent = () => (<div className="space-y-3">
      {/* Dev Mode Toggle */}
      ...
      <ScrollArea className="h-[500px] pr-4">
        ...
        <>
              {filteredClasses.map((group) => (<div key={group.category}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {group.classes.map((cls) => (<Button key={cls} variant="outline" size="sm" onClick={() => handleQuickStyle(cls)} className="text-xs h-6 px-2">
                        {cls}
                      </Button>))}
                  </div>
                </div>))}
        ...
        <>
              {filteredClasses.map((group) => (<div key={group.category}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {group.items.map((item) => (<Button key={item.value} variant="outline" size="sm" onClick={() => handleQuickStyle(item.value)} className="text-xs h-6 px-2" title={`Applies: ${item.value}`}>
                          {item.label}
                        </Button>))}
                  </div>
                </div>))}
    ...
    const TriggerButton = () => (<button data-custom-class-trigger className="h-8 w-8 flex items-center justify-center rounded-full shadow-lg hover:scale-110 transition-all bg-background border-2 border-border hover:border-primary" onMouseDown={(e) => {
    ...
    return (<AnimatePresence mode="wait">
      {position && (<motion.div key={position.top + position.left} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`${position ? "opacity-100" : "!opacity-0"} transition-opacity duration-300 absolute z-50 pointer-events-auto`} style={{
          ...
          {isMobile ? (
            ...
            <Sheet open={isOpen} onOpenChange={handleOpenChange}>
              ...
              <SheetContent side="bottom" className="h-[85vh] px-5 rounded-t-xl" onOpenAutoFocus={(e) => {
            ...
            <Popover open={isOpen} onOpenChange={setIsOpen}>
              ...
              <PopoverContent className="lg:w-[300px] max-h-[300px] overflow-y-auto" align="start" onOpenAutoFocus={(e) => {
