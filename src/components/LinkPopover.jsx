        ...
        if (state.currentSelection && state.currentSelection.text.length > 0) {
            ...
            savedSelectionRef.current = {
                ...
                formats: {
                    ...
                    strikethrough: state.currentSelection.formats.strikethrough || false,
                    code: state.currentSelection.formats.code || false,
                },
      ...
      {position && (<motion.div key={position.top + position.left} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`${position ? "opacity-100" : "!opacity-0"} transition-opacity duration-300 absolute z-50 pointer-events-auto`} style={{
          ...
          <Popover open={isOpen} onOpenChange={setIsOpen}>
            <PopoverTrigger asChild>
              <button data-link-popover-trigger className={`h-8 w-8 flex items-center justify-center rounded-full shadow-lg hover:scale-110 transition-all bg-background border-2 ${hasExistingLink
                ? 'border-blue-500 text-blue-500'
                : 'border-border hover:border-primary'}`} onMouseDown={(e) => {
            ...
            <PopoverContent className="w-80" align="start" onOpenAutoFocus={(e) => {
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <Button onClick={handleApplyLink} disabled={!hrefInput.trim()} size="sm" className="flex-1">
                    <LinkIcon className="size-3.5 mr-1.5"/>
                    {hasExistingLink ? 'Update' : 'Add Link'}
                  </Button>
                  {hasExistingLink && (<Button onClick={handleRemoveLink} variant="destructive" size="sm">
