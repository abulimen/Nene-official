...
import { FileText, Sparkles, BookOpen, Briefcase, User, Zap, Plus, Loader2, } from "lucide-react";
import { getAllTemplateMetadata, getTemplateById } from "../lib/templates";
// Category icons
const categoryIcons = {
    productivity: Zap,
    creative: Sparkles,
...
export function TemplateSwitcherButton({ onTemplateChange, currentState, }) {
    ...
    return (<>
      ...
      <div className="fixed bottom-6 left-6 z-50">
        ...
        }} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className={cn("h-12 w-12 rounded-full shadow-lg transition-all duration-200", "bg-primary hover:bg-primary/90", "hover:shadow-xl hover:scale-105", "group")} size="icon" title="Switch Template">
          <Plus className={cn("h-5 w-5 transition-transform duration-200", isHovered && "scale-110")}/>
        </Button>
      </div>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-6xl min-w-fit max-h-[85vh] overflow-hidden flex flex-col backdrop-blur-xl bg-background/95 border-border/50 shadow-2xl">
          ...
          <div className="flex flex-wrap gap-2 pb-6 border-b border-border/50">
            ...
            const Icon = category.value === "all" ? BookOpen : categoryIcons[category.value];
            const isActive = selectedCategory === category.value;
            return (<Button key={category.value} variant={isActive ? "default" : "outline"} size="sm" onClick={() => setSelectedCategory(category.value)} className={cn("gap-2 transition-all duration-200", isActive && "shadow-md scale-105")}>
          ...
          <div ref={scrollContainerRef} className="flex-1 overflow-y-auto pt-4">
            ...
            return (<button key={template.id} onClick={() => handleTemplateSelect(template.id)} disabled={isApplying} className={cn("group relative p-6 rounded-xl border border-border/60", "hover:border-primary/60 hover:shadow-lg hover:shadow-primary/10", "hover:bg-accent/30 hover:scale-[1.02]", "transition-all duration-300 ease-out", "backdrop-blur-sm bg-background/50", "text-left", isApplying && !isCurrentlyApplying && "opacity-50 cursor-not-allowed", isCurrentlyApplying && "border-primary/80 shadow-lg shadow-primary/20 scale-[1.02]")}>
                    {/* Category badge */}
                    ...
                    {/* Loading overlay */}
                    {isCurrentlyApplying && (<div className="absolute inset-0 bg-background/80 backdrop-blur-sm rounded-xl flex items-center justify-center z-10">
            ...
            {filteredTemplates.length === 0 && (<div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
