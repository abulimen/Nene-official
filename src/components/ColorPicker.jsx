...
export function ColorPickerComponent({ disabled, onColorSelect, selectedColor, }) {
    ...
    return (<Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" disabled={disabled} className="gap-1 md:gap-2 h-7 md:h-8 px-2 md:px-3">
        ...
        <Tabs defaultValue="preset" className="w-full">
          ...
          <TabsContent value="preset" className="space-y-3">
            ...
            <div className="grid grid-cols-5 gap-2">
              {presetColors.map((color) => (<button key={color.hex} onClick={() => onColorSelect(color.hex)} className={`h-10 rounded-md border-2 transition-all hover:scale-110 ${selectedColor === color.hex
                ? 'border-foreground'
                : 'border-border'}`} title={color.name} style={{ color: color.hex }}>
          ...
          <TabsContent value="custom" className="space-y-3">
            ...
            <ShadcnColorPicker defaultValue="#3b82f6" onChange={handleCustomColorChange} className="w-full rounded-md border bg-background p-4 shadow-sm">
