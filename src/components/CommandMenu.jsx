...
import { Heading1, Heading2, Heading3, Heading4, Heading5, Heading6, Type, Code, Quote, List, ListOrdered, Image, Video, Table, } from 'lucide-react';
...
export function CommandMenu({ isOpen, onClose, onSelect, anchorElement, nodeId, onUploadImage, onUploadVideo }) {
    ...
    const handleSelect = useCallback(async (commandValue) => {
    ...
    return (<Popover open={isOpen} onOpenChange={(open) => {
      ...
      <PopoverContent side="bottom" align="start" className="w-[320px] p-0" onOpenAutoFocus={(e) => {
        ...
        <Command ref={commandRef} shouldFilter={false}>
          <CommandInput placeholder="Search commands..." value={search} onValueChange={setSearch} autoFocus/>
          <CommandList>
            <CommandEmpty>No commands found.</CommandEmpty>
            <CommandGroup heading="Turn into">
              {filteredCommands.map((command, index) => (<CommandItem key={command.value} value={command.value} onSelect={() => handleSelect(command.value)} className={`
                  ...
                  <div className="flex flex-col">
                    ...
                    {command.description && (<span className="text-xs text-muted-foreground">
                        {command.description}
                      </span>)}
                  </div>
                </CommandItem>))}
