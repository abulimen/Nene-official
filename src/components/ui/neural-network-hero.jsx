...
`;
const CPPNShaderMaterial = shaderMaterial({ iTime: 0, iResolution: new THREE.Vector2(1, 1) }, vertexShader, fragmentShader);
...
function ShaderBackground() {
    ...
    return (<div ref={canvasRef} className="bg-black absolute inset-0 -z-10 w-full h-full" aria-hidden>
...
export default function Hero({ title, description, badgeText = "Generative Surfaces", badgeLabel = "New", ctaButtons = [
    ...
    return (<section ref={sectionRef} className="relative h-screen w-screen overflow-hidden flex flex-col">
      <ShaderBackground />
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col w-full items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
          {ctaButtons.map((button, index) => (<a key={index} href={button.href} target={button.target} rel={button.target === "_blank" ? "noopener noreferrer" : undefined} onClick={(e) => {
                if (button.onClick) {
                    e.preventDefault();
                    button.onClick();
                }
        ...
        <ul ref={microRef} className="mt-8 flex flex-wrap gap-6 text-xs font-extralight tracking-tight text-white/60">
          {microDetails.map((detail, index) => {
            ...
            return (<li key={index} ref={refMap[index]} className="flex items-center gap-2">
      ...
      <div className="inset-x-0 bottom-6 flex justify-center pointer-events-auto mt-auto pb-10">
        <div className="text-center text-xs text-white/50 font-light tracking-tight">
          ...
          <div className="flex items-center justify-center gap-3 flex-wrap">
            <a href="mailto:minamelad232@gmail.com" className="hover:text-white/70 transition-colors underline-offset-4 hover:underline">
              minamelad232@gmail.com
            </a>
            <span>|</span>
            <a href="https://github.com/Mina-Massoud/Mina-Rich-Editor" target="_blank" rel="noopener noreferrer" className="hover:text-white/70 transition-colors underline-offset-4 hover:underline">
              GitHub
            </a>
            <span>|</span>
            <a href="https://mina-massoud.com" target="_blank" rel="noopener noreferrer" className="hover:text-white/70 transition-colors underline-offset-4 hover:underline">
              Portfolio
            </a>
            <span>|</span>
            <a href="https://www.linkedin.com/in/mina-melad/" target="_blank" rel="noopener noreferrer" className="hover:text-white/70 transition-colors underline-offset-4 hover:underline">
              LinkedIn
            </a>
          </div>
        </div>
