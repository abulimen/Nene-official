...
import { EditorActions, isTextNode, } from "../lib";
import { useEditorStore, useEditorDispatch, useContainer, useSelectionManager, } from "../lib/store/editor-store";
...
// Import all handlers
import { createHandleSelectionChange, createHandleFormat, createHandleApplyColor, createHandleApplyFontSize, createHandleTypeChange, } from "../lib/handlers/selection-handlers";
import { createHandleKeyDown, createHandleContentChange, createHandleClickWithModifier, } from "../lib/handlers/keyboard-handlers";
import { createHandleImageDragStart, createHandleBlockDragStart, createHandleDragEnter, createHandleDragOver, createHandleDragLeave, createHandleDrop, } from "../lib/handlers/drag-drop-handlers";
import { createHandleFileChange, createHandleMultipleFilesChange, createHandleImageUploadClick, createHandleMultipleImagesUploadClick, createHandleFreeImageFileChange, createHandleFreeImageUploadClick, } from "../lib/handlers/file-upload-handlers";
import { createHandleNodeClick, createHandleDeleteNode, createHandleAddBlock, createHandleCreateNested, createHandleChangeBlockType, createHandleInsertImageFromCommand, createHandleCreateList, createHandleCreateListFromCommand, createHandleCreateLink, createHandleCreateTable, createHandleCopyHtml, createHandleCopyJson, } from "../lib/handlers/node-operation-handlers";
import { createHandleToggleImageSelection, createHandleClearImageSelection, createHandleGroupSelectedImages, checkImagesInSameFlex, createHandleReverseImagesInFlex, createHandleExtractFromFlex, } from "../lib/handlers/image-selection-handlers";
import { createHandleFlexContainerDragOver, createHandleFlexContainerDragLeave, createHandleFlexContainerDrop, } from "../lib/handlers/flex-container-handlers";
import { Block } from "./Block";
import { CoverImage } from "./CoverImage";
import { ExportFloatingButton } from "./ExportFloatingButton";
import { TemplateSwitcherButton } from "./TemplateSwitcherButton";
export function Editor({ readOnly: initialReadOnly = false, onUploadImage, onUploadVideo, notionBased = true, onNotionBasedChange, } = // enableVirtualization = false,
 {}) {
    ...
    const contentUpdateTimers = useRef(new Map());
    ...
    const [dragOverFlexId, setDragOverFlexId] = useState(null);
    const [flexDropPosition, setFlexDropPosition] = useState(null);
    const [tableDialogOpen, setTableDialogOpen] = useState(false);
    const [tableInsertionTargetId, setTableInsertionTargetId] = useState(undefined);
    ...
    const currentNode = activeNodeId
        ? container.children.find((n) => n.id === activeNodeId)
        : container.children[0];
    // Debounced dispatch for selection state updates
    const selectionDispatchTimerRef = useRef(null);
    ...
    const selectionParams = {
        ...
        state: {
            ...
            history: [container],
            currentSelection,
        }, // Minimal state object for handlers
        dispatch,
        selectionManager,
        nodeRefs,
    };
    ...
    const fileUploadParams = {
        container,
        dispatch,
        state: { activeNodeId, historyIndex, history: [container] },
        toast,
        setIsUploading,
        fileInputRef,
    ...
    // Create all handlers
    const handleSelectionChange = useCallback(createHandleSelectionChange(selectionParams, selectionDispatchTimerRef), [container, activeNodeId, selectionManager, dispatch]);
    const handleFormat = useCallback(createHandleFormat(selectionParams), [
        container,
        dispatch,
        selectionManager,
    ]);
    const handleApplyColor = useCallback(createHandleApplyColor(selectionParams, toast, setSelectedColor), [dispatch, selectionManager, toast]);
    const handleApplyFontSize = useCallback(createHandleApplyFontSize(selectionParams, toast), [dispatch, selectionManager, toast]);
    const handleTypeChange = useCallback(createHandleTypeChange(selectionParams, handleSelectionChange), [dispatch, selectionManager, handleSelectionChange]);
    const handleToggleImageSelection = useCallback(createHandleToggleImageSelection(selectedImageIds, setSelectedImageIds), [selectedImageIds]);
    const handleContentChange = useCallback(createHandleContentChange({
        container,
        dispatch,
        nodeRefs,
        lastEnterTime,
        onToggleImageSelection: handleToggleImageSelection,
    }, contentUpdateTimers), [container, dispatch, handleToggleImageSelection]);
    const handleKeyDown = useCallback(createHandleKeyDown({
        container,
        dispatch,
        nodeRefs,
        lastEnterTime,
    ...
    const handleClickWithModifier = useCallback(createHandleClickWithModifier({
        container,
        dispatch,
        nodeRefs,
        lastEnterTime,
    ...
    const handleNodeClick = useCallback(createHandleNodeClick({ container, dispatch }), [container, dispatch]);
    const handleDeleteNode = useCallback(createHandleDeleteNode({ container, dispatch, toast }), [container, dispatch, toast]);
    const handleAddBlock = useCallback(createHandleAddBlock({ dispatch, nodeRefs }), [dispatch, nodeRefs]);
    const handleCreateNested = useCallback(createHandleCreateNested({ container, dispatch, toast }), [container, dispatch, toast]);
    const handleChangeBlockType = useCallback(createHandleChangeBlockType({ dispatch, nodeRefs }), [dispatch, nodeRefs]);
    const handleInsertImageFromCommand = useCallback(createHandleInsertImageFromCommand({ dispatch, nodeRefs }, fileInputRef), [dispatch, fileInputRef]);
    const handleCreateList = useCallback(createHandleCreateList(nodeOperationParams), [container, dispatch, toast, editorContentRef]);
    const handleCreateListFromCommand = useCallback(createHandleCreateListFromCommand({ dispatch, toast, nodeRefs }), [dispatch, toast, nodeRefs]);
    const handleCreateLink = useCallback(createHandleCreateLink(nodeOperationParams), [container, dispatch, toast, editorContentRef]);
    const handleCreateTable = useCallback(createHandleCreateTable(nodeOperationParams, tableInsertionTargetId), [container, dispatch, toast, editorContentRef, tableInsertionTargetId]);
    const handleCreateTableFromCommand = useCallback((nodeId) => {
    ...
    const handleImportMarkdownTable = useCallback((table) => {
        ...
        if (targetNode) {
            ...
            dispatch(EditorActions.replaceContainer({
                ...container,
    ...
    const handleCopyHtml = useCallback(() => createHandleCopyHtml({ toast }, enhanceSpaces, setCopiedHtml)(container), [container, enhanceSpaces, toast]);
    const handleCopyJson = useCallback(() => createHandleCopyJson({ toast }, setCopiedJson)(container), [container, toast]);
    ...
    const handleDragOver = useCallback(createHandleDragOver({
        container,
        dispatch,
        draggingNodeId,
        setDraggingNodeId,
    ...
    const handleDragLeave = useCallback(createHandleDragLeave(setDragOverNodeId, setDropPosition), []);
    const handleDrop = useCallback(createHandleDrop(dragDropParams, dropPosition), [container, dispatch, toast, draggingNodeId, dropPosition, onUploadImage]);
    const handleFileChange = useCallback(createHandleFileChange(fileUploadParams), [container, dispatch, activeNodeId, toast, onUploadImage]);
    const handleMultipleFilesChange = useCallback(createHandleMultipleFilesChange(fileUploadParams), [container, dispatch, activeNodeId, toast, onUploadImage]);
    const handleImageUploadClick = useCallback(createHandleImageUploadClick(fileInputRef), []);
    const handleMultipleImagesUploadClick = useCallback(createHandleMultipleImagesUploadClick(multipleFileInputRef), []);
    const handleVideoUploadClick = useCallback(createHandleImageUploadClick(videoInputRef), []);
    const handleVideoFileChange = useCallback(createHandleFileChange(videoUploadParams), [container, dispatch, activeNodeId, toast, onUploadImage]);
    const handleFreeImageFileChange = useCallback(createHandleFreeImageFileChange(freeImageUploadParams), [container, dispatch, activeNodeId, toast, onUploadImage]);
    const handleFreeImageUploadClick = useCallback(createHandleFreeImageUploadClick(freeImageInputRef), []);
    const handleInsertComponentClick = useCallback(() => {
        setInsertComponentModalOpen(true);
    }, []);
    const handleInsertComponentSelect = useCallback((componentId) => {
        if (componentId === "free-image") {
            handleFreeImageUploadClick();
        }
        // Future: handle other component types here
    }, [handleFreeImageUploadClick]);
    const handleClearImageSelection = useCallback(createHandleClearImageSelection(setSelectedImageIds), []);
    const handleGroupSelectedImages = useCallback(createHandleGroupSelectedImages({ container, dispatch, toast }, selectedImageIds, handleClearImageSelection), [container, dispatch, toast, selectedImageIds, handleClearImageSelection]);
    ...
    const handleReverseImagesInFlex = useCallback(createHandleReverseImagesInFlex({ container, dispatch, toast }, selectedImageIds, flexInfo.flexParentId || ""), [container, dispatch, toast, selectedImageIds, flexInfo.flexParentId]);
    const handleExtractFromFlex = useCallback(createHandleExtractFromFlex({ container, dispatch, toast }, selectedImageIds, flexInfo.flexParentId || "", handleClearImageSelection), [
        container,
        dispatch,
        toast,
        selectedImageIds,
    ...
    const handleFlexContainerDragOver = useCallback(createHandleFlexContainerDragOver({
        container,
        dispatch,
        toast,
        draggingNodeId,
    ...
    const handleFlexContainerDragLeave = useCallback(createHandleFlexContainerDragLeave(setDragOverFlexId, setFlexDropPosition), []);
    const handleFlexContainerDrop = useCallback(createHandleFlexContainerDrop({
        container,
        dispatch,
        toast,
        draggingNodeId,
    ...
    useEffect(() => {
        const handlePaste = async (e) => {
            ...
            const isInEditor = Array.from(nodeRefs.current.values()).some((el) => el === activeElement || el.contains(activeElement));
            if (!isInEditor || readOnly)
                return;
            ...
            try {
                ...
                // Create media nodes
                const timestamp = Date.now();
                const mediaNodes = mediaUrls.map((url, index) => {
    ...
    useEffect(() => {
        const handleGlobalKeyDown = (e) => {
            ...
            if (isCtrlOrCmd && e.key === "a" && isInEditor) {
                ...
                if (!selection)
                    return;
                const currentBlock = activeElement;
    ...
    return (<div className="bg-background transition-colors flex flex-col flex-1 duration-300">
      {/* Editor with integrated toolbar */}
      <div className="mx-auto flex flex-col flex-1 w-full">
        <QuickModeToggle readOnly={readOnly} onReadOnlyChange={setReadOnly} notionBased={notionBased} onNotionBasedChange={onNotionBasedChange}/>
        {/* Toolbar - hidden in readOnly mode */}
        {!readOnly && (<EditorToolbar isUploading={isUploading} onImageUploadClick={handleImageUploadClick} onMultipleImagesUploadClick={handleMultipleImagesUploadClick} onVideoUploadClick={handleVideoUploadClick} onInsertComponentClick={handleInsertComponentClick} onCreateList={handleCreateList} onCreateTable={() => setTableDialogOpen(true)}/>)}
        <div className="shadow-2xl py-0 relative flex flex-col flex-1 rounded-none border-2 gap-3 transition-all duration-300">
          {/* Table Dialog */}
          <TableDialog open={tableDialogOpen} onOpenChange={(open) => {
            setTableDialogOpen(open);
            // Clear the target ID when dialog closes
            if (!open) {
                setTableInsertionTargetId(undefined);
            }
        }} onCreateTable={handleCreateTable} onImportMarkdown={handleImportMarkdownTable}/>

          {/* Insert Components Modal */}
          <InsertComponentsModal open={insertComponentModalOpen} onOpenChange={setInsertComponentModalOpen} onSelect={handleInsertComponentSelect}/>

          {/* Hidden file inputs for image and video uploads */}
          {!readOnly && (<>
              <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileChange} className="hidden"/>
              <input ref={multipleFileInputRef} type="file" accept="image/*" multiple onChange={handleMultipleFilesChange} className="hidden"/>
              <input ref={videoInputRef} type="file" accept="video/*" onChange={handleVideoFileChange} className="hidden"/>
              <input ref={freeImageInputRef} type="file" accept="image/*" onChange={handleFreeImageFileChange} className="hidden"/>
            </>)}

          {/* Editor Content */}
          <CardContent className={`px-0 flex flex-col w-full flex-1 transition-all duration-300 mx-auto ${readOnly ? "py-14 md:py-20" : ""}`}>
            <div ref={editorContentRef} className="h-full">
              <>
                ...
                <div data-editor-content className={`${notionBased && coverImage
            ? "pt-[280px] lg:pt-[420px]"
            : notionBased
                ? "pt-[50px]"
                : "pt-4"} px-10 transition-all mx-auto max-w-6xl duration-300`}>
            ...
            return (<div className="mx-auto max-w-6xl w-full" key={nodeKey}>
          ...
          {container.children
            ...
            .map((node) => (<FreeImageBlock key={node.id} node={node} isActive={activeNodeId === node.id} onClick={() => handleNodeClick(node.id)} onDelete={readOnly ? undefined : () => handleDeleteNode(node.id)} readOnly={readOnly}/>))}
        </div>
      </div>
      ...
      {!readOnly && (<SelectionToolbar selection={currentSelection} selectedColor={selectedColor} onFormat={handleFormat} onTypeChange={(type) => handleTypeChange(type)} onColorSelect={handleApplyColor} onFontSizeSelect={handleApplyFontSize}/>)}

      {/* Group Images Button - Floats when multiple images selected */}
      {!readOnly && (<GroupImagesButton selectedCount={selectedImageIds.size} inSameFlex={flexInfo.inSameFlex} onGroup={handleGroupSelectedImages} onReverse={flexInfo.inSameFlex ? handleReverseImagesInFlex : undefined} onExtract={flexInfo.inSameFlex ? handleExtractFromFlex : undefined} onClear={handleClearImageSelection}/>)}

      {/* Floating Export Button */}
      {!readOnly && (<ExportFloatingButton container={container} onCopyHtml={handleCopyHtml} onCopyJson={handleCopyJson} copiedHtml={copiedHtml} copiedJson={copiedJson} enhanceSpaces={enhanceSpaces} onEnhanceSpacesChange={setEnhanceSpaces}/>)}

      {/* Template Switcher Button - Bottom Left */}
      {!readOnly && (<TemplateSwitcherButton currentState={{
                activeNodeId,
                historyIndex,
                history: [container],
                currentSelection,
