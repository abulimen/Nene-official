...
import { isTextNode, isContainerNode } from '../types';
import { uploadImage } from '../utils/image-upload';
import { findNodeAnywhere } from '../utils/editor-helpers';
/**
 * Handle image drag start
 */
export function createHandleImageDragStart(setDraggingNodeId) {
    return (nodeId) => {
 ...
 */
export function createHandleBlockDragStart(setDraggingNodeId) {
    return (nodeId) => {
        setDraggingNodeId(nodeId);
    };
}
 ...
 * Handle drag over
 */
export function createHandleDragOver(params) {
    return (e, nodeId) => {
        ...
        const isTargetImage = isTextNode(targetNode) && targetNode.type === "img";
        const isDraggingImage = draggingNode &&
            isTextNode(draggingNode) &&
            draggingNode.type === "img";
        // Check if target and dragging nodes are in the same flex container
        const inSameFlexContainer = targetResult.parentId &&
        ...
        if (isTargetImage && isDraggingImage) {
            ...
            if (inSameFlexContainer) {
                ...
                const parent = draggingResult?.parentId
                    ? container.children.find((c) => c.id === draggingResult.parentId)
                    : null;
                if (parent) {
 ...
 * Handle drag leave
 */
export function createHandleDragLeave(setDragOverNodeId, setDropPosition) {
    return (e) => {
        e.preventDefault();
        e.stopPropagation();
 ...
 */
export function createHandleDrop(params, dropPosition) {
    return async (e, nodeId) => {
        const { container, dispatch, toast, draggingNodeId, setDraggingNodeId, setDragOverNodeId, setDropPosition, setIsUploading, onUploadImage } = params;
        e.preventDefault();
        e.stopPropagation();
        ...
        if (draggedNodeId && draggingNodeId) {
            ...
            const draggingNode = draggingResult.node;
            const targetNode = targetResult.node;
            const inSameFlexContainer = draggingResult.parentId &&
            ...
            if (dropPosition === "left" || dropPosition === "right") {
                ...
                if (inSameFlexContainer &&
                    ...
                    dispatch(EditorActions.updateNode(parent.id, {
                ...
                if (isTextNode(draggingNode) && isTextNode(targetNode)) {
                    ...
                    const targetRootIndex = container.children.findIndex((n) => n.id === nodeId ||
                        (isContainerNode(n) &&
                            n.children.some((c) => c.id === nodeId)));
                    const draggingRootIndex = container.children.findIndex((n) => n.id === draggingNodeId ||
                        (isContainerNode(n) &&
                            n.children.some((c) => c.id === draggingNodeId)));
            ...
            if (draggingResult.parentId && draggingResult.parent) {
                ...
                const insertPos = dropPosition === "before" || dropPosition === "after"
                    ? dropPosition
                    : "after";
                ...
                if (remainingChildren.length === 1) {
                    ...
                    if (isTargetTheFlexContainer) {
                        ...
                        let referenceNodeId = null;
                        let referencePosition = insertPos === "before" ? "before" : "after";
                        if (parentIndex > 0) {
                        ...
                        if (referenceNodeId) {
                            ...
                            actions.push(EditorActions.insertNode(remainingChildren[0], referenceNodeId, referencePosition));
                        ...
                        if (isTargetBeforeFlex || isTargetAfterFlex) {
                            ...
                            if (isTargetBeforeFlex) {
                                ...
                                actions.push(EditorActions.insertNode(remainingChildren[0], draggingNodeId, "after"));
                            }
                            else {
                                // Inserting after flex, so remaining child should be before dragged node
                                actions.push(EditorActions.insertNode(remainingChildren[0], draggingNodeId, "before"));
                            }
                    ...
                    actions.push(EditorActions.updateNode(parent.id, {
            ...
            if (draggingNodeAtRoot) {
                ...
                const isDraggingImage = isTextNode(draggingNode) && draggingNode.type === 'img';
                const isTargetImage = targetNodeAtRoot && isTextNode(targetNodeAtRoot) && targetNodeAtRoot.type === 'img';
