 ...
 */
export function createHandleSelectionChange(params, selectionDispatchTimerRef) {
    return () => {
        const { container, state, dispatch, selectionManager, nodeRefs } = params;
        const selection = window.getSelection();
        const hasText = selection !== null &&
        ...
        if ((hasText || hasCursor) && selection) {
            ...
            if (!currentElement) {
                ...
                const freshCurrentNode = state.activeNodeId
                    ? findNodeById(container, state.activeNodeId)
            ...
            if (currentElement) {
                ...
                if (actualNodeId) {
                    ...
                    const actualNode = findNodeById(container, actualNodeId);
 ...
 */
export function createHandleFormat(params) {
    return (format) => {
 ...
 */
export function createHandleApplyColor(params, toast, setSelectedColor) {
    return (color) => {
 ...
 */
export function createHandleApplyFontSize(params, toast) {
    return (fontSize) => {
        const { dispatch, selectionManager, nodeRefs } = params;
        // Get fresh selection from ref
        const refSelection = selectionManager.getSelection();
 ...
 * Handle type change
 */
export function createHandleTypeChange(params, handleSelectionChange) {
    return (type) => {
        const { dispatch, selectionManager, nodeRefs } = params;
