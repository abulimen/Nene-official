 ...
 * Handle node click
 */
export function createHandleNodeClick(params) {
    return (nodeId) => {
 ...
 * Handle delete node
 */
export function createHandleDeleteNode(params) {
    return (nodeId) => {
        ...
        if (nodeToDelete) {
            if (isContainerNode(nodeToDelete)) {
                const firstChild = nodeToDelete.children[0];
        ...
        const parentContainer = container.children.find((child) => isContainerNode(child) &&
            child.children.some((c) => c.id === nodeId));
        if (parentContainer) {
            const containerNode = parentContainer;
            ...
            if (remainingChildren.length === 1) {
                ...
                const actions = [EditorActions.deleteNode(parentContainer.id)];
                if (containerIndex > 0) {
 ...
 * Handle add block
 */
export function createHandleAddBlock(params) {
 ...
 */
export function createHandleCreateNested(params) {
    return (nodeId) => {
        const { container, dispatch, toast } = params;
        const result = findNodeInTree(nodeId, container);
        if (!result)
 ...
 */
export function createHandleChangeBlockType(params) {
 ...
 */
export function createHandleInsertImageFromCommand(params, fileInputRef) {
    return (nodeId) => {
 ...
 */
export function createHandleCreateList(params) {
    return (listType) => {
 ...
 */
export function createHandleCreateListFromCommand(params) {
 ...
 * Handle create link
 */
export function createHandleCreateLink(params) {
    return () => {
        const { container, dispatch, toast, editorContentRef } = params;
        const timestamp = Date.now();
 ...
 * Handle create table
 */
export function createHandleCreateTable(params, activeNodeId) {
    return (rows, cols) => {
        const { container, dispatch, toast, editorContentRef } = params;
        const timestamp = Date.now();
        // Create header cells
        ...
        const bodyRows = Array.from({ length: rows }, (_, rowIdx) => {
            const cells = Array.from({ length: cols }, (_, colIdx) => ({
 ...
 * Handle copy HTML
 */
export function createHandleCopyHtml(params, enhanceSpaces, setCopiedHtml) {
 ...
 * Handle copy JSON
 */
export function createHandleCopyJson(params, setCopiedJson) {
    return async (container) => {
        const { toast } = params;
