...
import { getNodeTextContent, hasInlineChildren, isTextNode, } from "../types";
import { findNodeInTree } from "../utils/editor-helpers";
/**
 * Handle content change in a node
 */
export function createHandleContentChange(params, contentUpdateTimers) {
 ...
 */
export function createHandleClickWithModifier(params) {
    return (e, nodeId) => {
...
export function createHandleKeyDown(params) {
    return (e, nodeId) => {
        ...
        const actualNodeId = e.currentTarget.getAttribute("data-node-id") || nodeId;
        if (e.key === "Enter") {
            ...
            if (node.type === "li" || node.type === "ol") {
                ...
                dispatch(EditorActions.updateNode(actualNodeId, {
            ...
            if ((cursorAtStart && isNodeEmpty) || node.type === "br") {
                ...
                const nonImageBlocks = siblings.filter((n) => {
                    if (!isTextNode(n))
                        return true; // Container nodes are not images
                    return n.type !== "img";
                });
