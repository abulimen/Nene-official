 ...
 */
import { isTextNode, hasInlineChildren, } from "../types";
import { updateNodeById, deleteNodeById, insertNode, moveNode, cloneNode, findNodeById, } from "../utils/tree-operations";
/**
...
export function editorReducer(state, action) {
    switch (action.type) {
        case "UPDATE_NODE": {
            ...
            const newContainer = updateNodeById(currentContainer, id, () => updates);
            return addToHistory({
                ...state,
                metadata: {
                    ...state.metadata,
        ...
        case "INSERT_NODE": {
            ...
            const currentContainer = state.history[state.historyIndex];
            const newContainer = insertNode(currentContainer, targetId, node, position);
        ...
        case "MOVE_NODE": {
            ...
            const currentContainer = state.history[state.historyIndex];
            const newContainer = moveNode(currentContainer, nodeId, targetId, position);
            return addToHistory({
                ...state,
                metadata: {
                    ...state.metadata,
        ...
        case "DUPLICATE_NODE": {
            ...
            const newContainer = insertNode(currentContainer, id, clonedNode, "after");
            return addToHistory({
                ...state,
                metadata: {
                    ...state.metadata,
        ...
        case "BATCH": {
            ...
            return actions.reduce((currentState, batchAction) => editorReducer(currentState, batchAction), state);
        }
        ...
        case "APPLY_INLINE_ELEMENT_TYPE": {
            ...
            console.log("📍 Target node:", nodeId, "Range:", start, "-", end, "Element type:", elementType);
            const currentContainer = state.history[state.historyIndex];
            const node = findNodeById(currentContainer, nodeId);
        ...
        case "TOGGLE_FORMAT": {
            ...
            if (!hasInlineChildren(node) &&
                ...
                console.log("⚠️ Using selection text as content source:", state.currentSelection.text);
            }
            // Build new children array by splitting segments that overlap with selection
            const newChildren = [];
            let currentPos = 0;
        ...
        case "APPLY_CUSTOM_CLASS": {
            ...
            if (!hasInlineChildren(node) &&
                ...
                state.currentSelection.text) {
                children = [{ content: state.currentSelection.text }];
                console.log("⚠️ [APPLY_CUSTOM_CLASS] Using selection text as content source:", state.currentSelection.text);
            }
            // Build new children array by splitting segments that overlap with selection
            const newChildren = [];
            let currentPos = 0;
        ...
        case "APPLY_INLINE_STYLE": {
            ...
            if (!hasInlineChildren(node) &&
                ...
                state.currentSelection.text) {
                children = [{ content: state.currentSelection.text }];
                console.log("⚠️ [APPLY_INLINE_STYLE] Using selection text as content source:", state.currentSelection.text);
            }
            // Build new children array by splitting segments that overlap with selection
            const newChildren = [];
            let currentPos = 0;
        ...
        case "APPLY_LINK": {
            ...
            if (!hasInlineChildren(node) &&
                ...
                state.currentSelection.text) {
                children = [{ content: state.currentSelection.text }];
                console.log("⚠️ [APPLY_LINK] Using selection text as content source:", state.currentSelection.text);
            }
            // Build new children array by splitting segments that overlap with selection
            const newChildren = [];
            let currentPos = 0;
        ...
        case "REMOVE_LINK": {
            ...
            if (!hasInlineChildren(node) &&
                ...
                state.currentSelection.text) {
                children = [{ content: state.currentSelection.text }];
                console.log("⚠️ [REMOVE_LINK] Using selection text as content source:", state.currentSelection.text);
            }
            // Build new children array by splitting segments that overlap with selection
            const newChildren = [];
            let currentPos = 0;
        ...
        case "DELETE_SELECTED_BLOCKS": {
            ...
            const newChildren = currentContainer.children.filter((child) => !state.selectedBlocks.has(child.id));
 ...
 * ```
 */
export function createInitialState(container) {
