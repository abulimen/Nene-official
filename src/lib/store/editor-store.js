...
const EMPTY_SELECTED_BLOCKS = new Set();
// Create the store with subscribeWithSelector middleware for fine-grained subscriptions
export const useEditorStore = create()(subscribeWithSelector((set, get) => {
 ...
 */
export function useBlockNode(nodeId) {
 ...
 */
export function useIsNodeActive(nodeId) {
 ...
 */
export function useContainerChildrenIds() {
 ...
 */
export function useContainerGetter() {
    return useEditorStore(useShallow((state) => () => state.history[state.historyIndex]));
}
/**
...
export function useSelection() {
    ...
    const [selection, setSelection] = React.useState(selectionManager.getSelection());
    React.useEffect(() => {
...
export function EditorProvider({ children, initialContainer, initialState, onChange, debug = false, }) {
