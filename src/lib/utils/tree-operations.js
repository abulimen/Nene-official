 ...
 */
import { isContainerNode, isStructuralNode } from "../types";
/**
 ...
 * ```
 */
export function findParentById(node, targetId) {
    if (!isContainerNode(node) && !isStructuralNode(node)) {
        return undefined;
    }
 ...
 * ```
 */
export function deleteNodeById(node, targetId) {
 ...
 * ```
 */
export function insertNode(root, targetId, newNode, position) {
...
function insertNodeRecursive(node, targetId, newNode, position) {
    ...
    // Recursively search in children
    const newChildren = node.children.map((child) => insertNodeRecursive(child, targetId, newNode, position));
    // Only create new object if children changed
    const childrenChanged = newChildren.some((newChild, index) => newChild !== node.children[index]);
    if (childrenChanged) {
        return {
 ...
 * ```
 */
export function moveNode(root, nodeId, targetId, position) {
 ...
 * ```
 */
export function traverseTree(node, callback, depth = 0) {
    callback(node, depth);
    if (isContainerNode(node) || isStructuralNode(node)) {
        for (const child of node.children) {
